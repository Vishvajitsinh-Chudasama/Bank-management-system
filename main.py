...
USER_TRANSECTION_FILE = "User_transection_file.json"
...
def Add_data_to_json_file(add_dict: dict|list, File_name: str) -> bool :
    ...
    elif File_name == USER_TRANSECTION_FILE:
...
def show_account_detail(Account_number : int) -> None:
    ...
    if None == User_data:
        ...
        print(f"Bank balance : {User_dict["Bank_Balnace"]}")

    input("Press enter continue.....")

def show_Transection_hestory(Account_number : int) -> str:
    """
    This function show transection history of specific account where account number is parameter of this function
    and return transection history.
    """
    message = ""

    data = Read_file_of_user_detail(USER_TRANSECTION_FILE)
                    ...
                    if None != User_data:
                        ...
                        if bcrypt.checkpw(User_password.encode(),User_dict["password"].encode()):
                            while True:
                                ...
                                print("4. Transfer Money to another anccount.")
                                print("5. Show the transection")
                                ...
                                match User_option:
                                    ...
                                    case 2:
                                        ...
                                        if None != mail_otp:
                                            ...
                                            if mail_otp == otp:
                                                old_password = maskpass.advpass("Enter you password to change password : ")
                                                if bcrypt.checkpw(old_password.encode(),User_dict["password"].encode()):
                                    ...
                                    case 3:
                                        ...
                                        User_password = maskpass.advpass("Enter Your password : ")
                                        if bcrypt.checkpw(User_password.encode(),User_dict["password"].encode()) and User_dict["Bank_Balnace"]>=amount_to_debit:
                                            User_dict["Bank_Balnace"] -= amount_to_debit
                                            if Add_data_to_json_file(User_dict,User_detail_file):
                                                ...
                                                if Add_data_to_json_file(Transaction,USER_TRANSECTION_FILE):
                                                    print(f"{amount_to_debit} is debited from {User_account_number}.")
                                                    print(f"remanig bank balance is {User_dict['Bank_Balnace']}")
                                        else:
                                            if User_dict["Bank_Balnace"]<amount_to_debit:
                                                print("Not enough amount avaliable.")
                                            else:
                                                print("try again.")
                                        input("Press enter to continue.....")
                                    case 4:
                                        ...
                                        amount_to_debit = int(input("Enter amount to be transer : "))
                                        User_password = maskpass.advpass("Enter Your password : ")
                                        if bcrypt.checkpw(User_password.encode(),User_dict["password"].encode()) and User_dict["Bank_Balnace"]>=amount_to_debit:
                                            ...
                                            if None != another_user_data:
                                                ...
                                                another_dict["Bank_Balnace"] += amount_to_debit
                                                User_dict["Bank_Balnace"] -= amount_to_debit
                                                if Add_data_to_json_file(User_dict,User_detail_file) & Add_data_to_json_file(another_dict,User_detail_file):
                                                    ...
                                                    if Add_data_to_json_file(Transaction_debit,USER_TRANSECTION_FILE) & Add_data_to_json_file(Transaction_credit,USER_TRANSECTION_FILE):
                                            ...
                                            if User_dict["Bank_Balnace"]<amount_to_debit:
                                                ...
                                                print("try again.")
                                    case 5:
                                        print(space(f"Transction Histroy of Account number {User_account_number}"))
                                        message = show_Transection_hestory(User_account_number)
                                        print(message)
                                        mail = input("If you want transection history to mail the press\"y\" : ")
                                        if mail == "y" | mail == "Y":
                                    ...
                                    case 6:
                                        ...
                                        User_password = maskpass.advpass("Enter Your password : ")
                                        if bcrypt.checkpw(User_password.encode(),User_dict["password"].encode()):
                                            User_dict["Bank_Balnace"] -= amount_to_credit
                                            if Add_data_to_json_file(User_dict,User_detail_file):
                ...
                if is_valid_email(User_email_id):
                    ...
                    while True:
                        if User_otp == otp:
                            ...
                            User_dict = {"Account_number": User_account_number, "First_name": User_first_name, "Last_name": User_last_name, "password": Hased_Account_password.decode(), "Bank_Balnace": User_Bank_Balance, "User_Email_id": User_email_id}
                            if Add_data_to_json_file(User_dict,User_detail_file):
